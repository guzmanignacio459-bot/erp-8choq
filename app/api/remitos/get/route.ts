...
export async function GET(req: Request) {
  try {
    ...
    if (!id) return json({ ok: false, error: "Falta parámetro id" }, 400);
    if (!GS_URL) return json({ ok: false, error: "GS_URL/APPS_SCRIPT_URL no configurada" }, 500);
    ...
    const resp = await fetch(GS_URL, {
      ...
      body: JSON.stringify({ action: "getRemito", id, token: TOKEN }),
    }).catch((e) => {
    ...
    return json({ ok: true, data: payload.data }, 200);
  } catch (err: any) {
    const msg =
      err?.name === "AbortError"
