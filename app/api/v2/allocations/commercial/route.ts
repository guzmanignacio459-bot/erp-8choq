...
import {
  allocateTnOrdersCommercialBatch,
  summarizeValidationFailures,
} from "@/services/erp-v2-allocations-commercial";
import type {
  V2CommercialAllocateRequest,
  V2CommercialAllocateResponse,
...
function json(body: V2CommercialAllocateResponse, status = 200): NextResponse {
...
function summarize(results: V2CommercialAllocateResponse["results"]) {
  return {
...
/**
 * POST /api/v2/allocations/commercial — prorrateo comercial TN (M4.2a, staging)
 *
 * Body: tnOrderId | tnOrderIds[], dryRun?
 * Gates: ERP_V2_DB_WRITE=true + Neon staging
 */
export async function POST(req: Request) {
  ...
  let body: V2CommercialAllocateRequest = {};
  try {
    body = (await req.json()) as V2CommercialAllocateRequest;
  } catch {
    return json(
      {
        ok: false,
  ...
  try {
    ...
    const results = await allocateTnOrdersCommercialBatch({
      tnOrderIds,
      dryRun,
    });
