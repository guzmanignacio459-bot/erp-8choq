#!/usr/bin/env node
/**
 * M6.5.1 — financial_account_assignments + periods schema push
 */
import fs from "fs";
...
const LABEL = "M6.5.1 db push";
const MIGRATION_DOC = path.join(
  process.cwd(),
  "docs/erp-m6.5.1-financial-account-assignments-migration.sql"
);
...
async function main() {
  ...
  if (sql) {
    fs.writeFileSync(
      MIGRATION_DOC,
      `-- M6.5.1 Financial Account Assignments — generated ${new Date().toISOString()}\n-- mode: ${mode}\n\n${sql}\n`
    );
