#!/usr/bin/env node
/**
 * M6.4 — financial_accounts schema push (staging Neon only)
 */
import fs from "fs";
...
const LABEL = "M6.4 db push";
const MIGRATION_DOC = path.join(
  process.cwd(),
  "docs/erp-m6.4-financial-accounts-migration.sql"
);
...
async function main() {
  ...
  if (sql) {
    fs.writeFileSync(
      MIGRATION_DOC,
      `-- M6.4 Financial Accounts — generated ${new Date().toISOString()}\n-- mode: ${mode}\n\n${sql}\n`
    );
